...
fn plan(sexp: &SExp, available_width: i32) -> PrintPlan {
  match sexp {
    ...
    SExp::List(es, _) => {
      ...
      let monoline_width = 1 + elem_plans.iter().map(|x| x.width()).sum::<i32>() + ((es.len() - 1) as i32) + 1;
      if available_width == 0 || monoline_width <= available_width {
        ...
        // multi-line:
        let ml_elem_plans: Vec<PrintPlan> =  es.iter().map(|x| plan(x, available_width - INDENT_WIDTH)).collect();
...
fn print_impl(sexp: SExp, plan: PrintPlan, indent: i32) {
  match (sexp, plan) {
    ...
    (SExp::List(es, bookend_style), PrintPlan::List(_, es_pps, linebreak)) => {
      let es_len = es.len();
      let insert_padding_space =
        if let PrintPlan::List(_, _, ListPrintPlan::Multiline) = es_pps[0] {
